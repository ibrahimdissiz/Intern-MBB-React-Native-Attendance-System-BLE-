...
export default class Listviewscreen extends Component {
    state = {
      ...
      _renderEmpty = () => {
        ...
        let text = '';
        if (!scanning && !ranging && !monitoring) text = "BEACON SINYALLERINI ALABILMEK ICIN TARAMAYA BASLA";
        if (scanning && !discoveredBeacons.length) text = "HENUZ BEACON BULUNAMADI...";
        if (ranging && !rangedBeacons.length) text = "HENUZ BEACON MESAFESINE GIRILEMEDI...";
        if (monitoring && !monitoredRegionsCloseBy.length) text = "YAKININIZDA IZLENEBILEN BOLGE YOK...";
        return text ? (
        ...
        return (
          <View style={styles.container}>
            <View style={styles.buttonContainer}>
              {this._renderButton('KESIFE BASLA', this._startDiscovery, '#84e2f9')}
              {this._renderButton('DUR', this._stopDiscovery, '#84e2f9')}
              {this._renderButton('YENILE', this._restartDiscovery, '#84e2f9')}
            </View>
            <View style={styles.buttonContainer}>
              {this._renderButton('SIRALAMAYA BASLA', this._startRanging, '#ABE88D')}
              {this._renderButton('DUR', this._stopRanging, '#ABE88D')}
              {this._renderButton('YENILE', this._stopAllRanging, '#ABE88D')}
            </View>
            <View style={styles.buttonContainer}>
              {this._renderButton('GOZETLEMEYE BASLA', this._startMonitoring, '#F48661')}
              {this._renderButton('DUR', this._stopMonitoring, '#F48661')}
              {this._renderButton('HEPSINI DURDUR', this._stopAllMonitoring, '#F48661')}
            </View>
            <View style={styles.buttonContainer}>
              {this._renderButton('DURUM BILGISI AL', this._getAuthorizationStatus, '#F4ED5A')}
              {this._renderAuthorizationStatusText()}
            </View>
            <View style={styles.buttonContainer}>
              {this._renderButton('SIRALANAN ALANLAR', this._getRangedRegions, '#ABE88D')}
              {this._renderButton('GOZETLENEN ALANLAR', this._getMonitoredRegions, '#F48661')}
            </View>
            {this._renderRegions()}
            <ScrollView>
              {this._renderEmpty()}
